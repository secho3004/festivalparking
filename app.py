...
def plot_chart_1():
    ...
    query = "SELECT 시도, 축제수, 총주차구획수 FROM region_summary ORDER BY 축제수 DESC"
    df = run_query(query)
    ...
    st.info("**SQL:** `SELECT 시도, 축제수, 총주차구획수 FROM region_summary ORDER BY 축제수 DESC`  \n"
...
def plot_chart_2():
    ...
    fig.add_trace(go.Scatter(x=df['월'], y=df['평균기간'], name='평균 기간(일)',
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.info("**SQL:** `SELECT 월, COUNT(*), AVG(기간_일수) FROM festival WHERE 월 IS NOT NULL GROUP BY 월`  \n"
...
def plot_chart_3():
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.info("**SQL:** `SELECT 시도, 주차장구분, COUNT(*) FROM parking WHERE 시도 IS NOT NULL GROUP BY 시도, 주차장구분`  \n"
            "**인사이트:** 대부분의 지역에서 공영 주차장 비중이 압도적으로 높습니다. "
            "민영 주차장 비율이 낮은 지역은 축제 시 민간 협력 주차 공간 확보가 어려울 수 있습니다.")
...
def plot_chart_4():
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.info("**SQL:** `SELECT 시도, 축제수, ROUND(총주차구획수/축제수, 1) FROM region_summary WHERE 축제수 > 0`  \n"
            "**인사이트:** 우측 하단에 위치한 지역은 축제는 많지만 주차 인프라가 열악한 곳입니다. "
